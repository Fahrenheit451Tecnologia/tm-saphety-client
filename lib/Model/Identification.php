...
class Identification implements ModelInterface, ArrayAccess
{
    ...
    const DOCUMENT_TYPE_R_00_PN = 'R_00_PN';
    const DOCUMENT_TYPE_CIVIL_REGISTRY = 'CivilRegistry';
    const DOCUMENT_TYPE_IDENTITY_CARD = 'IdentityCard';
    const DOCUMENT_TYPE_CITIZENSHIP_CARD = 'CitizenshipCard';
    const DOCUMENT_TYPE_FOREIGNER_CARD = 'ForeignerCard';
    const DOCUMENT_TYPE_FOREIGNER_IDENTIFICATION = 'ForeignerIdentification';
    const DOCUMENT_TYPE_NIT = 'NIT';
    const DOCUMENT_TYPE_PASSPORT = 'Passport';
    const DOCUMENT_TYPE_FOREIGN_IDENTIFICATION_DOCUMENT = 'ForeignIdentificationDocument';
    const DOCUMENT_TYPE_NUIP = 'NUIP';
    ...
    {
        return [
            self::DOCUMENT_TYPE_R_00_PN,
            self::DOCUMENT_TYPE_CIVIL_REGISTRY,
            self::DOCUMENT_TYPE_IDENTITY_CARD,
            self::DOCUMENT_TYPE_CITIZENSHIP_CARD,
            self::DOCUMENT_TYPE_FOREIGNER_CARD,
            self::DOCUMENT_TYPE_FOREIGNER_IDENTIFICATION,
            self::DOCUMENT_TYPE_NIT,
            self::DOCUMENT_TYPE_PASSPORT,
            self::DOCUMENT_TYPE_FOREIGN_IDENTIFICATION_DOCUMENT,
            self::DOCUMENT_TYPE_NUIP,
        ];
    }
    ...
    public function listInvalidProperties()
    {
        ...
        if (!in_array($this->container['country_code'], $allowedValues)) {
            $invalidProperties[] = sprintf(
    ...
    {
        $allowedValues = $this->getCountryCodeAllowableValues();
        if (!is_null($country_code) && !in_array($country_code, $allowedValues)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'country_code', must be one of '%s'",
                    implode("', '", $allowedValues)
