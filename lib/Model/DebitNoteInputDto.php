...
class DebitNoteInputDto implements ModelInterface, ArrayAccess
{
     ...
     * Sets delivery_date
     *
     * @param string $delivery_date debit Note Delivery Date. Format {yyyy-mm-dd}
     *
     * @return $this
     */
     ...
     *
     * @param \Swagger\Client\Model\DocumentReferenceInputDto[] $document_references Documents(Sales Invoice, Exportation Invoice) refered to the debit note
     *
     * @return $this
     */
    ...
    public function setPdf($pdf)
    {
        if (!is_null($pdf) && (strlen($pdf) > 5000000)) {
            throw new \InvalidArgumentException('invalid length for $pdf when calling DebitNoteInputDto., must be smaller than or equal to 5000000.');
        }
        if (!is_null($pdf) && (!preg_match("/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/", $pdf))) {
            throw new \InvalidArgumentException("invalid value for $pdf when calling DebitNoteInputDto., must conform to the pattern /^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/.");
        }
